...
- **CLI completions are now isolated and inference-only.** Since the CLI providers are agentic and we bypass their directory-trust gate, a completion with no workdir used to inherit the daemon's cwd — so a prompt-injected model could, in principle, try file/shell tools against a real project. Now: a completion with no workdir runs in an **empty sandbox dir** (`~/.sokuza/cli-sandbox`) instead of the daemon's cwd, and **Codex completions run with `--sandbox read-only`** (no writes/network). Agent mode (`ai.agent`, which has a clone and opts into tools) is unchanged. claude-code/opencode/gemini already get no auto-approval in completion mode, so tools can't auto-execute headless.

### Changed

- **Refreshed Gemini model suggestions** to current IDs — `gemini-3.1-flash-lite` (the 3.x Flash tier) and `gemini-3-pro-preview`, then `gemini-2.5-flash`/`-lite`. (There is no `gemini-3.5-flash` / `gemini-3.x-flash` id; the 3.x Flash is the `-lite`.) The field stays free-text.
...
- **Revert PR review from `ai.agent` back to `ai.review` (completion) — restores reliability.** 0.2.0 rebuilt the PR-review template (and editor recipes) around the tool-using `ai.agent` node to investigate a clone. With glm-5.1 + opencode that collapsed JSON-reliability from ~94% (459/481 over May 1–19, and 131/140 May 20–Jun 2) to roughly **zero**: the model investigates and *trails off without ever emitting the structured JSON*, and `ai.agent`'s `parse_as_review` has no repair retry — so the raw investigation narration ("Let me look at… Now let me verify…") got posted as the "review." Root cause was the *harness*, not the model: the full agent loop (`--dangerously-skip-permissions`) never converges, whereas completion-mode `ai.review` answers and emits JSON directly (and fails *loudly* via its repair + empty-output guard rather than posting noise). `ai-pr-review.yaml` and the Manual/Auto recipes now wire `clone + fetch-diff → ai.review → comment`.
- **Revert the diff-only `ai.review` prompt change from 0.2.0.** It instructed the model to ignore on-disk code, which fights the restored clone. Back to the original prompt that ran at ~95% with a clone as cwd.

### Added

- **`ai.review` node gains a `workdir` input port.** Wire a `github-clone-repo` path into it to run the review (completion mode) with the clone as cwd, so the model can read the surrounding code to verify findings — the clone-aware review the pre-graph steps template had, which the steps→graph rewrite (`4c6ba7b`) dropped. The `ai-pr-review` template and the editor recipes set the node's `timeout: 0` so a clone-aware review isn't cut off at the 600s node cap; the workflow `queue.timeout` governs.
...
- **`ai-pr-review` template clones the PR again — agentic review restored.** The steps→graph rewrite in the visual-editor overhaul (`4c6ba7b`) silently dropped the `github-clone-repo` node and the `workdir: {{steps.clone.path}}` wiring when it converted `templates/ai-pr-review.yaml` from `steps:` to `graph:` form. The model was left with no checkout, yet the prompt still told it to "investigate / read the files" — so under opencode (an agent runtime) it would wander the daemon's working directory (an unrelated repo), burning its turn and intermittently producing no review. The template now wires `trigger → clone → fetch-diff → ai.agent(parse_as_review) → comment`: the agent gets the real PR checkout in its `workdir` to investigate, plus the authoritative diff embedded in its prompt (the shallow clone can't compute the diff, and `ai.agent` doesn't read the `context` port). The sibling `auto-fix-pr-review` template was never converted and kept its clone, so it was unaffected.- **`ai.review` (diff-only) prompt no longer invites open-ended investigation.** The diff-only review system prompt told the model "if you are an agentic CLI … you MAY perform a small number of focused file reads, greps, or searches first" and to "investigate (read the code, check callers)" / "Read `CLAUDE.md`". But `ai.review` ships the full diff in the prompt and has no PR checkout — so under opencode (an agent runtime) `glm-5.1` would wander the daemon's working directory (an unrelated repo), burning its whole turn and sometimes ending without emitting JSON. The result was intermittent `returned no usable content` / `produced no JSON even after repair` failures. The prompt is now internally consistent: review strictly from the provided diff, no tools, emit JSON immediately. Because the investigation clause lived in `STANDARD_OUTPUT_FORMAT`, the `buildRepairPrompt` path — which already said "Do not use tools" — no longer contradicts itself either. The agentic `ai.agent` path (its own inline prompt) is unaffected.
...
- **`github-poll` `initialize()` now resets `state` alongside the existing lifecycle-flag and `orgRepos` resets.** Sister fix to the earlier reuse-path hardening. The `orgRepos = new Map()` reset added in 0.1.8 actively defeated `refreshOrgRepos`'s previousUnion-vs-currentUnion pruning diff on the very next refresh: with `orgRepos` already empty, `allRepos()` returns just `explicitRepos`, so repos that dropped out of the config don't appear in `previousUnion` and never get `pruneState()`'d. Stale entries in `state.seededRepos` and the `state.lastXxxIds` maps then survived across config reloads — and on a subsequent reload that re-added the org, `seededRepos.has(repo)` still returned `true`, so `firstSight` evaluated `false` against a possibly weeks-old snapshot, emitting transition events for every change that happened across the reload gap. `initialize()` now replaces `this.state` with empty maps along the same lines as the `orgRepos` reset — with per-repo seeding the re-seed is silent (no event emission, just re-snapshots state), so the cost is one poll cycle's API budget per reload for a clean baseline that matches the "fully equivalent to fresh construction for everything that varies with config" intent stated in the comment.
- **`github-poll` rejects non-positive / non-finite `interval` and `org_refresh` values at init.** Both numeric config fields used to accept `0`, negatives, `NaN`, and `Infinity` without validation. `setInterval(fn, 0)` (or any value Node coerces to ~1ms) fires every tick — the integration's re-entrance guards kept work from overlapping but each tick still allocated a Promise and burned CPU. `NaN` slipped past the `??` defaults downstream because `NaN ?? x === NaN`, then the `* 1000` was still `NaN`, then `setInterval` treated `NaN` as `0`. `initialize()` now rejects any non-`undefined` value that isn't a finite positive number with the same error shape as the existing `repos`/`orgs` validation — misconfigs surface at init time, not silently as a hot poll loop.
