...
- **`github-poll` integration auto-enumerates orgs.** New optional `orgs: [my-org]` config alongside the existing `repos:` list — the integration calls `/orgs/{org}/repos` at startup and merges the result into the watch set. Periodic re-enumeration (default 1h, configurable via `org_refresh`) picks up newly-created repos without a restart and drops repos that were removed or made private (per-org sets are REPLACED, not accumulated). Failures for one org log + keep the previous set; one bad org doesn't blank the others. Validation widened to require at least one of `repos` or `orgs`. Pairs naturally with the trigger-side `repo: my-org/*` globbing shipped in PR #4 — the engine glob filters down to the workflow's scope after the poller surfaces events from the whole enumerated set. Closes the poller half of A10 in [NODE_GRAPH_SUPPORT.md](NODE_GRAPH_SUPPORT.md). `gh-cli` integration unchanged — its `gh search prs --author @me` path is structurally different and would need its own redesign for the all-author org-watching use case.
