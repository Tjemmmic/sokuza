...
- **`sokuza service restart` command and dashboard button.** Bounces the autostart service from a single place so users can recover a stuck instance without having to remember the platform-specific incantation. Linux runs `systemctl --user restart sokuza.service`, macOS runs `launchctl kickstart -k gui/<uid>/ai.sokuza` (falling back to the `user/<uid>` domain for headless macOS installs), Windows runs `schtasks /End` followed by `schtasks /Run` on the registered task. Each branch refuses cleanly when the service isn't installed instead of failing silently. The dashboard's System page surfaces a "Restart service" button next to "Disable autostart" — backed by `POST /api/system/service/restart`, which validates installation up front (returns 400 if not installed) and defers the actual restart by a tick via `setImmediate` so the 202 reply flushes before the platform service manager delivers SIGTERM to the responding process. The dashboard handler tolerates the kill-mid-response race (treats `Failed to fetch` / `NetworkError` as benign), and re-renders the System page after a short delay so the user sees the restarted service's state. The post-update CLI hint now points at the new single command instead of three platform-specific ones.
