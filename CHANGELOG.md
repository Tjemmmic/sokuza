...
- **`ai.review` now falls back to the next provider in the chain when the primary produces no parseable JSON after its repair budget.** Real-world failure mode driving this fix: opencode + `zai-coding-plan/glm-5.1` as the primary provider produced a stream of tool-narration / exploration prose with no JSON, the repair pass produced the same shape, and the PR #13 hard-reject (correctly) refused to post the prose as a review — but the workflow then failed with `ai-review: opencode (zai-coding-plan/glm-5.1) produced no JSON even after 1 repair attempt(s)` because the chain-level fallback wrapper (`runCompletionWithFallback`) only escalated on *runtime* errors (provider down, exit code). A successful HTTP response that happened to be unparseable was treated as terminal. Now `ai-review` builds the chain itself (`[primary, ...fallback_providers]`) and runs the full `initial completion → repair loop → parse` sequence per provider. A provider that emits narration is treated the same as a provider that threw at the network level: log, advance, try the next. A user with `anthropic` as a fallback now succeeds with anthropic's review when opencode flakes, instead of the workflow failing outright. Behavior preserved unchanged: `parse_repair_retries: 0` still posts the primary provider's raw text (legacy contract for callers who explicitly opted out of repair-or-fallback semantics); the empty-output guard, the no-JSON-after-repair diagnostic message, and the run-record shape all match prior versions so downstream tooling and `/produced no JSON/` log greps keep working. Chain-exhausted errors now list every provider tried (`exhausted N provider(s) in fallback chain (opencode → anthropic → claude-code)`) so the user can see which combination flaked.
